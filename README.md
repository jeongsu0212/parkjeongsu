...
-----------------------------------------------------------------------------
####1204
- 다국어지원하는 문자셋을 만들때: 유니코드로 제작. \u0041 => 유니코드에디터에서는 A로 수정합니다.
- 자바프로그램중 반복문: 향상된 for문을 시작합니다.
- 참조형변수: 객체가 위치해 있는 주소 값을 저장한다. = 실행가능한 변수 = 인스턴스변수 = 메모리에 로딩된 변수
- 참조형변수예1: String s3 = new String(); // 인스턴스변수를 이런 방식 망들어서 사용함.

- new String()처럼 클래스이름과 똑같은 매서드를 생성자 매서드라고한다.
- 참조형변수예2: s3 = "일반변수값입니다.";
- 기본형변수예: String s3 = "일반변수값입니다.";
- 객체지향프로그래밍: Object Oriented Programming 인스턴스(실행가능한클래스)를 사용하는 프로그램.
- new 키워드로 만드는 오브젝트를 사용하는 방식이 필수.(한물간 방식)
