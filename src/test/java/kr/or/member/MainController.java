...
public class MainController {
	...
	public static void main(String[] args) throws IOException {
		...
		memberVO.setName("박정수");
		memberVO.setAge(27);
		memberVO.setPhoneNum("010-3308-0000");
		...
		members[1] = memberVO2;
		members[2] = memberVO3;
		// 전에 사용했던 출력보다는 개선된 방식으로 출력(아래) MemberService 서비스 전용 클래스만들엇 처리.
		...
		//네트워크로 서버시간을 가져올때 접속하는 응답이 1초를 넘기면 재접속해라.
		timeClient.open();
		//http://pool.ntp.org/는 서버시간보내줄 실제 서버 주소.
		InetAddress address = InetAddress.getByName("pool.ntp.org");
		TimeInfo timeInfo = timeClient.getTime(address);
		//서버시간을 담은 timeinfo 객체변수를 사용(아래) 변수는 카멜표기법*
		//TimeStamp 1970년부터 초단위로 계산된 현재까지의 초를 합친결과값(아래) 48321574982초
		long returnTime =  timeInfo.getMessage().getTransmitTimeStamp().getTime();
		//위 반환받은 타임스탬프초 값을 사람이 알아볼수 있는 시간으로 변환(아래)
		Date nowDate = new Date(returnTime);
		//System.out.println("현재 서버시간은-영어버전  " + nowDate);
		//Wed Dec 09 11:18:04 KST 2020 -> 2020-12-09 11:26:05 AM
