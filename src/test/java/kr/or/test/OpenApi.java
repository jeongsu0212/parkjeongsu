...
/**
 * OpenApi클래스로 HRD-Net에서 제공하는 통합과정 구직훈련과정API 목록을 출력하는 자바앱
 * @author 박정수
 *
 */
public class OpenApi {
	...
	public static void serviceAip() {
		...
		try {
			...
			try {
				...
				System.out.println("버퍼리더로 읽어들인 최종 결과값은 아래입니다. while반복회수는" + cnt);
	...
	//스태틱 메서드는 new키워드로 객체오브젝트 생성없이 바로 접근이 가능한 메서드를 말합니다.
	public static void main(String[] args) {
		...
		exec.scheduleAtFixedRate(new Runnable() {//스레드를 이용해서 5초단위로 메서드를 반복실행.
			public void run() {
				serviceAip();
			}
		...
		//StaticTest.test();//스태틱메서드는 오브젝트생성없이 클래스로 바로 접근이 가능 메서드, 변수도 가능.
		/*
		int sum =0;
		while(sum < 10) {
...
class StaticTest {//스태틱은 메서드, 변수에도 해당되는 이론
	public static void test() {//클래스를 읽어들이는 순간 실행이 가능.
		//스태틱메서드는 메서드를 읽어들이는 순간 고정메모리를 할당받음.=실행가능.
