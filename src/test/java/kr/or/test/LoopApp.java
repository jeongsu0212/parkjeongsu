...
public class LoopApp {
	...
	public static void main(String[] args) {
		...
		//for문(반복의 시작값정의;반복의 종류값정의;반복할때 증가하는값) {반복할떄 구현내용}
		...
		//int length = names.length;//length기본형변수에는 5가 들어가게됩니다.
		//names는 클래스형 변수지만, 인스턴스는 아님.
		char name = '사';//임베디드장비와 자바프로그램 통신시 이것을 사용합니다.
		//임베디드 프로그램에서 char을 사용하는 이유는 메모리를 효율적으로 사용하기 위해서 char를 사용.
		//자바에서 String형 큰따옴표는 문자열데이터 감싸줄때, char형 작은따옴표는 1개문자데이터를 감싸줄때.
		//배열[] 1차원배열 = 가로선 1row 하나의 줄
		...
		String[][] members = {
				{"admin","관리자","admin@abc.com","trun","2020-12-04","ROLE_ADMIN"},
				{"user","사용자1","user@abc.com","false","2020-12-04","ROLE_USER"}
		};
		//배열타입[][] 2차원배열 = 표 1row, 2row 리스트가 있는 줄
		//위에 members 2차원배열 변수가 노랗게 나오는 이유는, 정의는 했는데, 사용하지 않아서 그럼.
		//아래쪽에 사용하면 노란언더라인이 없어짐.
		System.out.println("======회원리스트 입니다.======");
		System.out.println("user_id | user_name | email use | regdate | level");
