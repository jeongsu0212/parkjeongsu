		...
		$.ajax({
			...
			success:function(result){
				if(result=="success") {
					...
					replyList();//댓글리스트 메서드호출 댓글영역html재생성.
...
$(document).ready(function(){
	$("#updateReplyBtn").on("click", function(){
		var rno = $("#rno").val();//modal창의 input태그값 변수
		var reply_text_modal = $("#reply_text_modal").val();//modal창의 input태그값 변수 지정
		$.ajax({
			type:"patch",
...
$(document).ready(function() {
	$("#insertReplyBtn").on("click", function() {//댓글등록버튼을 클릭했을 때 구현내용(아래)
		...
		//RestAPI서버단으로 보낼 변수값 정의
		var bno = "${boardVO.bno}";//java EL 변수값
		var reply_text = $("#reply_text").val();//input 태그값
		var replyer = $("#replyer").val();//댓글폼 input 태그값
		//alert(bno + ":" + replyer + ":" + reply_text);//디버그
		//return false;//디버그용 강제 중지
		if(reply_text == "" || replyer == "") {
			//boolean타입 1x0=0,1x1=1,0x1=0 (&& = and 두개다 true이어야지만 모두 ture)
			//boolean타입 1+0=1,1+1=1,0+0=1 (|| = or둘중에 하나라도 true이면 모두 ture)
