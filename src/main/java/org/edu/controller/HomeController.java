...
public class HomeController {
	...
	public String board_delete(HttpServletRequest request, RedirectAttributes rdat, @RequestParam("bno") Integer bno, @RequestParam("page") Integer page) throws Exception {
		...
		if(!delFiles.isEmpty()) {
			for(AttachVO file_name:delFiles) {
				...
				if(target.exists()) {
					target.delete();//실제 업로드된 파일지우기
				}
			}
		}
		//DB에서 부모 게시판에 댓글이 있다면 댓글삭제처리 후 게시글 삭제처리-서비스에 내장(아래)
		boardService.deleteBoard(bno);
		rdat.addFlashAttribute("msg", "삭제");//msg변수값은 URL에 표시가 나오지안게 숨겨서 board_list로 보냄.
		return "redirect:/home/board/board_list?page="+page;//쿼리스트링변수는 URL에 표시가 됨.
	}
	...
	public String mypage_update(HttpServletRequest request,MemberVO memberVO,RedirectAttributes rdat) throws Exception {
		...
		rdat.addFlashAttribute("msg", "회원수정");//model로 값을 보내지 못하는 이유는 redirect이기때문.
		return "redirect:/member/mypage";
	}
	//사용자 홈페이지 회원 마이페이지 접근 매핑
	@RequestMapping(value="/member/mypage",method=RequestMethod.GET)
	public String mypage(HttpServletRequest request, Model model) throws Exception{
		//마이페이지는 로그인상태에서만 접근가능하기 때문에 로그인 세션변수중 로그인 아이디변수 session_userid
		HttpSession session = request.getSession();
