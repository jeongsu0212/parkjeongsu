...
public class NaverLoginController {
	 ...
	 * response_type: 네이버App응답에 대한 테이터 타입
	 * redirect_uri: 네이버 로그인 인증결과를 전달받는 콜백URL
	 * state: 네이버App이 생성한 토큰(네트웍에서 전송되는 자료단위-인증정보)의 상태
	 */
	private final static String CLIENT_ID = "DkXiIFVBe_DgZ6X3lz48";
	...
	public String getAuthorizationUrl(HttpSession session) {
		...
		OAuth20Service oauthService = new ServiceBuilder()
				...
				.state(state)
				.build(NaverLoginApi.instance());//instance바로호출 static으로 정
		return oauthService.getAuthorizationUrl();
	}
	
	/* 네아로 Callback 처리 및 Access Token 구하기 메서드 */
	public OAuth2AccessToken getAccessToken(HttpSession session, String code, String state) throws IOException {
		/* 콜백URL로 전달받은 세션 검증용 난수값state와 세션에 저장되어있는 값이 일치하는지 확인 */
	...
	private void setSession(HttpSession session, String state) {
		...
		session.setAttribute(SESSION_STATE, state);//고유 uuid값이 세션으로 저장
	}
