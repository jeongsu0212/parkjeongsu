...
public class CommonController {
	...
	@Inject
	IF_BoardDAO boardDAO;//첨부파일을 개별 삭제하기 위해서 인젝트함.
	...
	public String getUploadPath() {//파일 삭제시 컨트롤러에서 호출로 사용
		return uploadPath;
	}
	...
	@ResponseBody //이 애노테이션으로 지정된 메서드는 페이지 이동처리가 아니고, RestAPI처럼 현재 페이지에 구현결과내용을 전송받음.
	public FileSystemResource download(
		...
		real_file_name = URLEncoder.encode(real_file_name, "UTF-8").replaceAll("\\+", "%20"); //(\\ 패턴, "%20" 변경값)
		//위에 URL인코더는 파일명이 한글(일본,베트남어)일때 깨지는 것을 방지
	...
	public String fileUpload(MultipartFile file) throws IOException {
		...
		return saveFileName;//copy로 업로드 이후에 저장된 real_file_name 스트링문자열값을 1개를 반환
	}
	...
	@ResponseBody //메서드 응답을 내용만 반환받겠다는 명시
